...
public class CelluleMulti {
	...
	public void checkTheRandom( Users[] tab, int nb){
		for (int i =0; i<nb*2;i++){
			...
			if (tab[i].getRandomNBTrame() <= 0){
				...
				tab[i].setMoyNBpaq(r.nextInt(4)+1);
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		for (int DivUser = 0; DivUser<5; DivUser++){
			for (int nbUser = 1; nbUser<9; nbUser++){
			...
			cell.resultatFinal = new double[8]; 
			cell.rempliTramePourcentage = new double[8];
			cell.resultatEnergie = new double[8];
		}
		// fin de DivUser
