...
public class CelluleBitParUR {
	...
	public Users allocAlea(ArrayList<Users> list){
		Random generator = new Random();
		int indice = generator.nextInt(list.size());
		Users tmp = list.remove(indice);
		list.add(tmp);
		return tmp;
	}
	...
	public void checkTheRandom( Users[] tab, int nb){
		for (int i =0; i<nb*2;i++){
			...
			if (tab[i].getRandomNBTrame() <= 0){
				...
				tab[i].setMoyNBpaq(r.nextInt(50)+1);
			}
		}
	}
	...
	public void menage() {
		...
		// On vide la liste de bit par trame
		this.ListeBitParTrame.clear();
	...
	public static void main(String[] args) {
		...
		CelluleBitParUR cell = new CelluleBitParUR();
		...
		for (int DivUser = 1; DivUser<11; DivUser++){
				System.out.println("C'est parti pour "+ nbUser *2 + " utilisateurs, avec la diversité " + DivUser);
				System.out.println("*********************************************");
				System.out.println("*********************************************");
				...
				while (i != 0) {
					...
					cell.varierDebit();
					//cell.roundRobin(cell.utilisateursBesoin);
					...
					for (Users u : cell.utilisateursBesoin) {
						...
						double nbp = (num * 10) + moypaquet;
						while(nbp < 0) {
							generator = new Random();
							num = generator.nextGaussian();
							nbp = (num * 10 ) + moypaquet;
						}
						/*if(x%500 == 0) {
					nbp = 1000;
				}*/
				...
				cell.rempliTramePourcentage[DivUser-1] = cell.moyenneListe(cell.remplitramePour1000);
				cell.BitParTrame[DivUser-1] = cell.moyenneListe(cell.ListeBitParTrame);
				double moyenne = 0 ;
			...
			//on sort du nbUser
			System.out.println("Global = "+cell.toString(cell.BitParTrame));
			...
			XYSeries serie1 = new XYSeries("Courbe Bit par RU ");
			...
			//XYSeries serieLoin= new XYSeries("Courbe Délai Loin");
				...
				for(int j = 0 ; j<cell.BitParTrame.length;j++){
					serie1.add(j+1, cell.BitParTrame[j]);
				}
			...
			//cell.resultatFinal = new double[5]; 
			...
			JFreeChart chart = ChartFactory.createXYLineChart
					("Bit par UR en fonction de la diversité", "Diversité", "Bit", Dataset, PlotOrientation.VERTICAL, true, true, false);
