		...
		for (int nbUser = 1; nbUser<11; nbUser++){
			...
			while (i != 0) {
				...
				for (Users u : cell.utilisateursBesoin) {
					...
					//System.out.println(nbp);
					System.out.println("nbp = " + nbp +"user = "+u.getID());
				...
				double URalloue =0;
				double Bitalloue = 0;
				double moyenneDebitUser0 =0;
				int nbPorteuseUser0 =0;
				int nbTimeSlotUser0 = 0;
				int timeSlot;
				int memtimeslot = -1;
				double PersonneReveille;
				for (timeSlot = 0;timeSlot < 5;timeSlot++){
					...
					for (int Porteuse = 0; Porteuse < 128;Porteuse++){					
						...
						if(proprio.getID()==IDfirstUser){
							moyenneDebitUser0 += proprio.getDebit();
							nbPorteuseUser0 += 1;
							if(memtimeslot != timeSlot){
								nbTimeSlotUser0 += 1;
								memtimeslot = timeSlot;
							}
							
							//System.out.println(proprio.getDebitMoyen());
						}
						URalloue++;
					...
					//System.out.println("Personne reveille = " +PersonneReveille + "avec la div = " + diversite + " et nbUSer = "+ nbUser*2);
					// Derniere porteuse.
				...
				System.out.println("conso total user" +IDfirstUser+" = " +moyenneDebitUser0 + "et il a eu " + nbPorteuseUser0 + "porteuse" + "sur "+ nbTimeSlotUser0 + "time slot");
				System.out.println("on est rendu au time Slot = "+timeSlot +" sont ils servit "+ (cell.utilisateursBesoin.isEmpty() && ListeUsersMax.isEmpty()));
				// On a fini la trame 
				if(i==9 || i==0){
				for (Users u : cell.delaiUtilisateurs.keySet()){
					System.out.println("Energie fin de trame = "+u.getEnergie() +"user = "+u.getID() +"et le timeSLot "+timeSlot +" et i = "+i);
