...
public class Users {
	...
	//mettre le cas si taille = 8 et somme ur = 9 (pour pas faire de nb n�gatif)
	public int soulager(){
		int res =0;
		//Cas o� sommeUR est >= 10
		if(this.SommeUR >= taillepaquet) {
			//Cas o� on a qu'un seul paquet  dans le buffer
			...
			//Cas o� on a plus d'un paquet dans le buffer
			else {
				this.SommeUR -= this.bufferToReceive.get(0).getTaille();
				int tmp = this.bufferToReceive.get(0).getTime();
				this.bufferToReceive.remove(0);
		...
		//Cas o� sommeUR est inf�rieur � 10
		else {
