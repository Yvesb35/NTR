...
public class CelluleMulti {
	...
	public static void main(String[] args) {
		...
		System.out.println("C'est parti pour "+ nbUser *2 + " utilisateurs, avec la diversitÈ " + (DivUser+1)*2);
		System.out.println("*********************************************");
		System.out.println("*********************************************");
		...
		while (i != 0) {
			...
			for (int timeSlot = 0;timeSlot < 5;timeSlot++){
				...
				// remet dans la liste des besoin les users qui n'ont pas ÈtÈ servis assez.
			}
			// On a fini la trame 
		...
		JFreeChart chartNBPaquets = ChartFactory.createXYLineChart
				("Energie moyen en fonction de la diversite", "DiversitÈ", "Energie moyenne par users", xyDatasetNBPaquets, PlotOrientation.VERTICAL, true, true, false);
		ChartFrame frameNBPaquets=new ChartFrame("Courbe Energie/DiversitÈ",chartNBPaquets);
		...
		JFreeChart chartRemplissage10Users = ChartFactory.createXYLineChart
			      ("D√©lai pour 10 utilisateurs", "TL", "Delai", xyDatasetRemplissage10Users, PlotOrientation.VERTICAL, true, true, false);
